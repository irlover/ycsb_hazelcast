...
import com.yahoo.ycsb.process.Prop;
...
public class StatusThread extends Thread {
	...
	public StatusThread(LoadThread lt, HashMap<String, Registry> rmiClients) {
		...
		//_printstatsinterval = 5;//bug

		_printstatsinterval = Config.getConfig().print_stats_interval;
	}
		...
		do {
			...
			if (rmiClients != null) {
				...
				while (itr.hasNext()) {
					...
					String key = itr.next();
					try {
						SlaveRMIInterface loadgen = (SlaveRMIInterface) rmiClients
								.get(key).lookup(SlaveClient.REGISTRY_NAME);
						...
						System.out.println("Could not get stats from " + key
								+ " because slave was not bound");
					} catch (RemoteException e) {
						System.out.println("Could not get stats from " + key
								+ " because slave is not running");
					}
		...
		try {
			...
			if (exportFile == null) {
				...
				out = new FileOutputStream(new File(
						Prop.getExportedResultsFolder(), exportFile), true);
			}
